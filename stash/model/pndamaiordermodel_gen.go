...
type (
	...
	PnDamaiOrder struct {
		...
		OrderStatus      int64          `db:"order_status"`       //  0：待付款 1：已付款 2：已发货 3：已取消 4:未补差 5：已退款 6:已完成
		...
		TotalBasePrice   float64        `db:"total_base_price"`   // 订单商品成本总价
		...
		CardPayPrice     string         `db:"card_pay_price"`     // 卡券支付金额
		CardPayOneCard   float64        `db:"card_pay_one_card"`  // 次卡支付金额
		...
		GoodsId          int64          `db:"goods_id"`           // 演出商品id
		SessionsName     string         `db:"sessions_name"`      // 场次名称
		SessionsId       int64          `db:"sessions_id"`        // 场次id
		PriceId          int64          `db:"price_id"`           // 价格id
		SeatData         lib.JsonNullString `db:"seat_data"`          // 座位信息
		IdSeat           int64          `db:"id_seat"`            // 0:未选座 1：选座
		DamaiPayResult   lib.JsonNullString `db:"damai_pay_result"`   // 大麦支付返回结果
		DamaiOrderSn     string         `db:"damai_order_sn"`     // 大麦订单号
		DamaiOrderStatus int64          `db:"damai_order_status"` // 大麦订单状态
		UserInfo         string         `db:"user_info"`          // 观影人信息
		StartTime        string         `db:"start_time"`         // 演出时间
		DeliveryTypes    int64          `db:"delivery_types"`     // 1和3为电子票，2和4为纸质票。
		PayLimitTime     int64          `db:"pay_limit_time"`     // 过期时间
		Num              int64          `db:"num"`                // 数量
		PayStatus        int64          `db:"pay_status"`         // 付款状态 0：待付款 1：已付款 2：未付款 3:支付失败 4:未补差价5：已完成
		IsHide           int64          `db:"is_hide"`            // 0:正常1：关闭
		WxResponse       lib.JsonNullString `db:"wx_response"`        // 微信回调结果
		ShowImg          lib.JsonNullString `db:"showImg"`            // 商品图片
		Price            float64        `db:"price"`              // 单价
		VenueName        lib.JsonNullString `db:"venue_name"`         // 场馆名称
		VenueAddress     lib.JsonNullString `db:"venue_address"`      // 场馆地址
		CityName         lib.JsonNullString `db:"city_name"`          // 城市名称
		WxPrice          float64        `db:"wx_price"`           // 微信补差金额
		IsSeat           int64          `db:"is_seat"`            // 1:有座位信息 0：没有座位信息
		OrderMaizuoSn    string         `db:"order_maizuo_sn"`    // 卖座订单号
		Platform         int64          `db:"platform"`           // 1：pc 2:手机
	}
)
